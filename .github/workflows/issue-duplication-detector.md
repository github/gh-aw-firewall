...
2. **Fetch the new issue**: Use the GitHub issue tools to get issue `${{ github.event.issue.number }}` from `${{ github.repository }}`, including its number, title, body, labels, and creation time.
...
4. **Search for potential duplicates via GitHub API**: Always search GitHub for issues with similar keywords, whether or not the cache had data:
   - Use the GitHub search tools to find up to 10 open issues matching the key terms in `${{ github.repository }}`
   - Use the GitHub issue tools to list up to 20 open issues, including number, title, body, and labels
...
- Use pagination and always specify a result limit when listing or searching issues