---
description: Smoke test for Copilot CLI using PAT-based auth — validates the COPILOT_GITHUB_TOKEN path through the api-proxy sidecar (no copilot-requests permission)
on:
  roles: all
  schedule: every 12h
  workflow_dispatch:
...
safe-outputs:
  ...
  add-labels:
    allowed: [smoke-copilot-pat]
  messages:
    footer: "> 🔑 *PAT report filed by [{workflow_name}]({run_url})*"
    run-started: "🔑 [{workflow_name}]({run_url}) is testing PAT-based auth on this {event_type}..."
...
steps:
  - name: Pre-compute smoke test data
    ...
    run: |
      ...
      TEST_FILE="$TEST_DIR/smoke-test-copilot-pat-${GITHUB_RUN_ID}.txt"
      mkdir -p "$TEST_DIR"
      echo "Smoke test passed for Copilot PAT at $(date)" > "$TEST_FILE"
