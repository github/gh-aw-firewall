...
- `COPILOT_PROVIDER_API_KEY=<azure-api-key>` (security-sensitive; use env var, not config)
- `COPILOT_PROVIDER_TYPE=azure` (optional; Azure is auto-detected from hostname)
- `COPILOT_AZURE_API_VERSION=2024-10-21` (optional; defaults to `2024-10-21`)

> **Note:** The legacy `COPILOT_API_KEY` is no longer used for BYOK credential
> resolution. Use `COPILOT_PROVIDER_API_KEY` paired with
> `COPILOT_PROVIDER_BASE_URL` as documented in the
