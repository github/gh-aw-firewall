...
- `container.mounts[]` → `-v, --mount <host_path:container_path[:mode]>` *(repeatable; format: `/host_path:/container_path[:ro|rw]`; both paths must be absolute; in chroot mode, container paths are automatically prefixed with `/host`)*
