...
run_case loopback-tcp 0 \
  'out=/tmp/loopback-tcp; timeout 10 nc -l -p 3002 >"$out" & listener=$!; trap "kill $listener 2>/dev/null || true" EXIT; sleep 1; printf loopback-ok | nc -w 3 127.0.0.1 3002; wait "$listener"; trap - EXIT; grep -q loopback-ok "$out"'
