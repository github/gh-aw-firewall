...
describe('self-hosted runner doctor workflow config', () => {
  ...
  it('keeps the shared catalog, workflow playbook, and portable agent aligned for new failure modes', () => {
    ...
    for (const content of [shared, portableAgent]) {
      ...
      // D1 gVisor resolved, Kata still open
      expect(content).toContain('**gVisor (`runsc`) resolved** in PR github/gh-aw-firewall#6093');
      ...
      expect(content).toContain('**Fixed in AWF (PR github/gh-aw-firewall#6072, merged 2026-07-10)**');
      expect(content).toContain('| `[WARN] Rootless artifact permission repair failed ... (exit 1)` with no stderr detail, followed by `Command completed with exit code: 1` despite apparent agent success | B11 |');
    }
