...
describe('smoke copilot byok aoai entra workflow output requirements', () => {
  it('requires noop fallback when no pull request context exists', () => {
    ...
    expect(source).toContain('**If triggered by a pull request**, add a **very brief** comment');
    expect(source).toContain('If all tests pass on a pull request trigger, add the label `smoke-copilot-byok-aoai-entra`');
