...
The repositories that build GitHub Agentic Workflows use agentic workflows for their own CI. Those workflows include [Auto-Triage Issues](https://github.com/github/gh-aw/blob/main/.github/workflows/auto-triage-issues.md?plain=1) for labeling new issues, [Contribution Check](https://github.com/github/gh-aw/blob/main/.github/workflows/contribution-check.md?plain=1) for auditing new pull requests againts contributor guidelines, [Test Quality Sentinel](https://github.com/github/gh-aw/blob/main/.github/workflows/test-quality-sentinel.md?plain=1) for reviewing test depth on every ready-for-review PR, [Glossary Maintainer](https://github.com/github/gh-aw/blob/main/.github/workflows/glossary-maintainer.md?plain=1) for keeping documentation in sync with code changes, and three daily quality checks: [Daily Syntax Error Quality](https://github.com/github/gh-aw/blob/main/.github/workflows/daily-syntax-error-quality.md?plain=1), [Daily Compiler Quality](https://github.com/github/gh-aw/blob/main/.github/workflows/daily-compiler-quality.md?plain=1), and [Daily Community Attribution](https://github.com/github/gh-aw/blob/main/.github/workflows/daily-community-attribution.md?plain=1). These run on production hardware against production API rate limits.
...
A [Daily Token Usage Auditor](https://github.com/github/gh-aw/blob/main/.github/workflows/daily-token-consumption-report.md?plain=1) reads token usage artifacts from recent workflow runs, aggregates consumption by workflow and time period, and posts a structured report. It flags workflows whose token usage footprint has increased since the last report, surfaces the most expensive workflows, and highlights anomalous runs such as a workflow that normally completes in 4 LLM turns taking 18.

A [**Daily Token Optimizer**](https://github.com/github/gh-aw/blob/main/.github/workflows/copilot-token-optimizer.md?plain=1) goes further. When the Auditor flags a heavy workflow,it examines the workflow's source and recent run logs, then creates a new issue describing concrete inefficiencies and recommended changes. It has consistently found inefficiencies that we had missed.
...
![Before vs. after optimization across 4 gh-aw workflows with sufficient data, measured in effective tokens](token-savings-chart-v2.png)
...
### Cross-repository validation: gh-aw-firewall

To check whether these patterns generalize beyond a single repository, we ran the same auditor–optimizer cycle on the [gh-aw-firewall](https://github.com/github/gh-aw-firewall) repository, which hosts the Agent Workflow Firewall—a separate codebase with its own set of production agentic workflows. We collected 2,881 token-usage records across seven optimization epochs spanning April 1–24.

![Before vs. after optimization across 3 firewall workflows, measured in effective tokens](token-savings-firewall.png)

Three workflows had enough data for comparison. **Security Guard**, which audits every PR for security-sensitive changes, dropped from 70 K to 28 K ET (−60%) after two rounds of prompt tightening and a relevance-gate that skips LLM analysis for PRs with no security-relevant file changes. **Smoke Claude**, an integration test that exercises the firewall's Claude CLI path, fell from 23 K to 5 K ET (−79%)—the largest relative reduction in either repository—driven primarily by a model switch from Sonnet to Haiku (0.25× multiplier) combined with MCP tool pruning. **Smoke Copilot**, the corresponding Copilot CLI smoke test, showed a more modest −29% (174 K → 124 K ET); this workflow's token budget is dominated by Copilot CLI's own internal reasoning, which is outside the workflow author's direct control.

The firewall results reinforce the patterns we observed in gh-aw. The deepest cuts come from eliminating structural waste—unnecessary LLM invocations (Security Guard's relevance gate) and oversized tool manifests (Smoke Claude's pruning)—rather than from fine-tuning prompts. Model-tier changes amplify these gains when the task complexity permits a lighter model.

From these results across both repositories, we highlight three patterns.

**Many agent turns are deterministic data-gathering.** Auto-Triage Issues shows the strongest sustained improvement in gh-aw (−44% across 62 post-fix runs) because the optimization eliminated structural inefficiency: many agent turns were spent on reads that required no inference, such as fetching issue metadata and scanning labels. Moving those reads into pre-agentic `gh` CLI steps before the agent starts removed them from the LLM reasoning loop entirely. The same pattern drove Security Guard's −60% reduction in gh-aw-firewall: a relevance gate now skips the LLM entirely for PRs that touch no security-sensitive files. The common principle is that the cheapest LLM call is the one you don't make.

Contribution Check illustrates a confounding factor: the same optimization pattern was applied (50–96% of turns were data-gathering), yet ET shows a slight *increase* (+9%). The cause is workload shift, not optimization failure: in the pre-optimization period 41% of runs processed small PRs (ET < 100 K) and 39% processed large PRs (ET > 300 K), while the post-optimization period—which coincided with a burst of development activity—had only 17% small PRs and 64% large PRs. Output tokens, which carry a 4× weight in the ET formula, rose 29% as the agent reviewed bigger diffs. The optimization likely improved per-turn efficiency, but the shift toward heavier workloads masks that gain in the aggregate numbers.
