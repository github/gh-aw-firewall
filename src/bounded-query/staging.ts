...
import { deriveSeedId, normalizeRepoKey, type BoundedQueryPaths } from './paths';
import { BOUNDED_QUERY_REPO_PATTERN } from './protocol';
import type { BoundedQuerySeed, BoundedQueryStagingResult } from './types';
 ...
 *   the agent/broker/query environment, or in any log line.
...
const ASKPASS_TOKEN_FILE_ENV = 'AWF_BOUNDED_QUERY_STAGING_TOKEN_FILE';
...
export class BoundedQueryStagingError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'BoundedQueryStagingError';
  }
}
...
export function buildCloneUrl(repo: string): string {
  if (!BOUNDED_QUERY_REPO_PATTERN.test(repo)) {
    throw new BoundedQueryStagingError(`Refusing to stage unsafe repository slug: ${repo}`);
  }
 ...
 * The file is created inside the (already 0o700) bounded query root.
 */
function writeAskpassHelper(root: string): string {
  const askpassPath = path.join(root, 'askpass.sh');
  const script = [
    '#!/bin/sh',
    '# Generated by AWF bounded-query staging. Reads the credential from a',
...
function verifySeedReadOnly(seedPath: string): void {
  ...
  if (offenders.length > 0) {
    throw new BoundedQueryStagingError(
...
function assertNoSubmodules(seedPath: string): void {
  ...
  if (fs.existsSync(gitmodules)) {
    throw new BoundedQueryStagingError(
      'Repository declares submodules (.gitmodules); bounded queries reject submodule-bearing repositories',
    );
  }
  ...
  if (fs.existsSync(modulesDir)) {
    throw new BoundedQueryStagingError(
      'Repository contains .git/modules; bounded queries reject submodule-bearing repositories',
    );
  }
}
...
export function scrubSeed(seedPath: string): void {
  ...
  if (gitDirStat.isSymbolicLink()) {
    throw new BoundedQueryStagingError('Refusing to stage a seed whose .git is a symlink');
  }
  if (!gitDirStat.isDirectory()) {
    // A file-form `.git` is a gitdir pointer into an external repository —
    // exactly the external reference bounded queries must never resolve.
    throw new BoundedQueryStagingError('Refusing to stage a seed whose .git is not a directory');
  }
...
async function stageOneSeed(
  ...
  params: Required<Pick<StageBoundedQuerySeedsParams, 'paths' | 'runId'>> & {
    gitRunner: GitRunner;
    gitEnv: NodeJS.ProcessEnv;
  },
): Promise<BoundedQuerySeed> {
  ...
  if (fs.existsSync(seedPath)) {
    throw new BoundedQueryStagingError(`Seed directory already exists: ${seedId}`);
  }
 ...
 * {@link BoundedQueryStagingError} is thrown so the caller aborts before the
 * primary agent starts.
 */
export async function stageBoundedQuerySeeds(
  params: StageBoundedQuerySeedsParams,
  ...
  for (const seed of seeds) {
    logger.debug(`Bounded queries: staged ${seed.repo} at ${seed.commit} (seed ${seed.seedId})`);
  }
