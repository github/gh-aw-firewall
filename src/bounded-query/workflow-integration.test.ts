...
/**
 * Lifecycle ordering guarantees for bounded queries.
 *
...
const baseConfig: WrapperConfig = {
  ...
  workDir: '/tmp/awf-bounded-query-workflow',
...
describe('bounded-query staging in the main workflow', () => {
  it('stages seeds before configs are written and containers start', async () => {
    ...
    expect(callOrder[0]).toBe('prepareBoundedQueries');
    expect(callOrder.indexOf('prepareBoundedQueries')).toBeLessThan(callOrder.indexOf('writeConfigs'));
    expect(callOrder.indexOf('prepareBoundedQueries')).toBeLessThan(callOrder.indexOf('startContainers'));
  });

  it('does not stage anything when bounded queries are disabled', async () => {
    const callOrder: string[] = [];
    const deps = createDeps(callOrder);
    ...
    expect((deps as unknown as { prepareBoundedQueries: jest.Mock }).prepareBoundedQueries).not.toHaveBeenCalled();
  });

  it('aborts before the primary agent runs when staging fails', async () => {
    const callOrder: string[] = [];
    const deps = createDeps(callOrder, {
      prepareBoundedQueries: jest.fn().mockRejectedValue(new Error('seed unavailable')),
    });
  ...
  it('stages before host iptables setup too, so no network state is created on failure', async () => {
    ...
    expect(callOrder.indexOf('prepareBoundedQueries')).toBeLessThan(callOrder.indexOf('ensureFirewallNetwork'));
  });

  it('refuses to run when bounded queries are enabled but no staging implementation was injected', async () => {
    const callOrder: string[] = [];
    const deps = createDeps(callOrder, { prepareBoundedQueries: undefined });
