...
declare const __AWF_BOUNDED_QUERY_WRAPPER__: string | undefined;

function loadWrapperSource(): string {
  if (typeof __AWF_BOUNDED_QUERY_WRAPPER__ !== 'undefined') {
    return __AWF_BOUNDED_QUERY_WRAPPER__;
  }
  ...
  throw new Error(`Bounded-query wrapper not found at ${candidates.join(' or ')}`);
}

/** Materializes the wrapper in the agent-only ingress root. */
export function writeBoundedQueryWrapper(paths: BoundedQueryPaths): string {
