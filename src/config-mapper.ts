...
export function mapAwfFileConfigToCliOptions(config: AwfFileConfig): Record<string, unknown> {
  ...
  return {
    ...
    nvxSignerWorkflow: config.nvx?.signerWorkflow,
    nvxOpenvmm: config.nvx?.openvmmPath,
    nvxKernel: config.nvx?.kernelPath,
    nvxInitramfs: config.nvx?.initramfsPath,
    nvxMemoryMib: config.nvx?.memoryMib,
    nvxMemoryMaxBytes: config.nvx?.memoryMaxBytes,
    nvxPidsMax: config.nvx?.pidsMax,
    nvxScratchBytes: config.nvx?.scratchBytes,
