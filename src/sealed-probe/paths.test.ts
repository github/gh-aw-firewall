...
describe('sealed-probe paths', () => {
  const workDir = '/tmp/awf-12345';

  it('derives every artifact path under a single sealed-probes subtree', () => {
    const paths = resolveSealedProbePaths(workDir);

    expect(paths.root).toBe(path.join(workDir, 'sealed-probes'));
...
describe('generateSealedProbeRunId', () => {
  it('produces a fresh 128-bit hex identifier', () => {
    const first = generateSealedProbeRunId();
    const second = generateSealedProbeRunId();
