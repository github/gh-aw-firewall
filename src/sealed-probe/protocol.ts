/**
 * Sealed-probe request/result protocol v2: a deliberately finite,
 ...
 * or probe output) can grow an unbounded parse tree, and duplicate object
 * keys — which `JSON.parse` would silently collapse — are rejected outright.
 *
 * `containers/sealed-probe/broker/protocol.js` is a deliberate,
 ...
 * `src/sealed-probe/protocol-parity.test.ts` runs shared vectors through
 * both and fails the moment they disagree.
 */

/** Wire protocol version. Only this exact value is accepted. */
export const PROBE_PROTOCOL_VERSION = 2;
 ...
 *
 * Keep in sync with `sealedProbes.privateRepos.items` in
...
export interface ObjectSchemaNode {
  ...
  readonly fields: readonly { name: string; schema: SealedProbeSchemaNode }[];
}
export interface TupleSchemaNode {
  readonly type: 'tuple';
  readonly items: readonly SealedProbeSchemaNode[];
}
export interface ArraySchemaNode {
  readonly type: 'array';
  readonly items: SealedProbeSchemaNode;
  readonly length: number;
}
export interface UnionSchemaNode {
  readonly type: 'union';
  readonly variants: readonly { tag: string; schema: SealedProbeSchemaNode }[];
}
 ...
 */
export type SealedProbeSchemaNode =
  | ConstSchemaNode
  | BooleanSchemaNode
  | EnumSchemaNode
  | IntegerSchemaNode
...
export type SealedProbeSchemaValidation =
  | { valid: true; schema: SealedProbeSchemaNode }
...
/**
 * Builds one validated {@link SealedProbeSchemaNode}, enforcing every finite
 ...
 */
function buildSchemaNode(raw: unknown, ctx: SchemaParseContext, depth: number): SealedProbeSchemaNode | undefined {
  ...
  switch (node.type) {
    ...
    case 'object': {
      ...
      const fields: { name: string; schema: SealedProbeSchemaNode }[] = [];
      for (const name of fieldNames) {
    ...
    case 'tuple': {
      ...
      const items: SealedProbeSchemaNode[] = [];
    ...
    case 'union': {
      ...
      const variants: { tag: string; schema: SealedProbeSchemaNode }[] = [];
      for (const tag of tags) {
 ...
 */
export function validateSchema(raw: unknown): SealedProbeSchemaValidation {
 ...
 */
export function schemaCardinality(schema: SealedProbeSchemaNode): bigint {
  switch (schema.type) {
    case 'const':
      return 1n;
    case 'boolean':
 ...
 */
export function queryBitsForSchema(schema: SealedProbeSchemaNode): number {
 ...
 */
export function validateValueAgainstSchema(schema: SealedProbeSchemaNode, value: unknown): boolean {
  switch (schema.type) {
    case 'const':
 ...
 * bytes a probe wrote — so two different serializations of the same
 * semantic value (whitespace, key order, numeric formatting) collapse to the
 * identical observable transcript.
 */
export function canonicalizeSchemaValue(schema: SealedProbeSchemaNode, value: unknown): string {
  switch (schema.type) {
    case 'const':
...
/** A sealed-probe execution request, already assembled from wire framing. */
...
export type SealedProbeValidation =
  | { valid: true; request: SealedProbeRequest }
  | { valid: false; errors: string[] };

/**
 * Validates an unknown value as a {@link SealedProbeRequest}: field shape,
 * the `privateRepo` slug pattern, the finite response schema, and the
 * script size cap.
 */
export function validateSealedProbeRequest(raw: unknown): SealedProbeValidation {
  if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
  ...
  } else if (privateRepo.length > MAX_PRIVATE_REPO_LENGTH || !SEALED_PROBE_REPO_PATTERN.test(privateRepo)) {
    errors.push(
...
/**
 * Parses and validates a probe's raw output file contents against the
 ...
 */
export function parseAndValidateProbeOutput(
  raw: string,
  schema: SealedProbeSchemaNode,
