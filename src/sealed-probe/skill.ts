...
import { SEALED_PROBE_SENSITIVITY_RUN_BITS } from '../types/sealed-probe-options';
import {
  AGENT_SKILL_PATH,
  PROBE_MOUNT_DIR,
...
function formatRunBudget(repo: SealedProbeRepository): string {
  const bits = SEALED_PROBE_SENSITIVITY_RUN_BITS[repo.sensitivity];
...
description: >-
  ...
  inside a sealed, offline sandbox and get back a value conforming to a
...
It must write its answer to \`${PROBE_MOUNT_DIR}/out\` as a single JSON value
...
Path("${PROBE_MOUNT_DIR}/out").write_text(json.dumps(found))
\`\`\`
...
- A repository whose remaining budget cannot afford even the cheapest
  ...
  ${RESULT_STATUS_BIT_COST + TIMING_BUCKET_BITS} bits) can no longer be probed
