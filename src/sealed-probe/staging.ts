 ...
 *   the agent/broker/probe environment, or in any log line.
...
const ASKPASS_TOKEN_FILE_ENV = 'AWF_SEALED_PROBE_STAGING_TOKEN_FILE';
...
export class SealedProbeStagingError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'SealedProbeStagingError';
  }
}
...
export function buildCloneUrl(repo: string): string {
  if (!SEALED_PROBE_REPO_PATTERN.test(repo)) {
    throw new SealedProbeStagingError(`Refusing to stage unsafe repository slug: ${repo}`);
  }
  return `https://github.com/${repo}.git`;
}

/**
 * Writes the staging token to a 0o600 file inside the (already 0o700) sealed
 * probe root.
 *
 ...
 * The file is created inside the (already 0o700) sealed probe root.
 */
function writeAskpassHelper(root: string): string {
  const askpassPath = path.join(root, 'askpass.sh');
  const script = [
    '#!/bin/sh',
    '# Generated by AWF sealed-probe staging. Reads the credential from a',
...
function verifySeedReadOnly(seedPath: string): void {
  ...
  if (offenders.length > 0) {
    throw new SealedProbeStagingError(
 ...
 * probe sandbox must never be able to resolve, and silently dropping it would
 * hand the probe a repository that does not match what the operator approved.
 */
function assertNoSubmodules(seedPath: string): void {
  const gitmodules = path.join(seedPath, '.gitmodules');
  if (fs.existsSync(gitmodules)) {
    throw new SealedProbeStagingError(
      'Repository declares submodules (.gitmodules); sealed probes reject submodule-bearing repositories',
    );
  }
  ...
  if (fs.existsSync(modulesDir)) {
    throw new SealedProbeStagingError(
      'Repository contains .git/modules; sealed probes reject submodule-bearing repositories',
    );
  }
}
...
export function scrubSeed(seedPath: string): void {
  ...
  if (gitDirStat.isSymbolicLink()) {
    throw new SealedProbeStagingError('Refusing to stage a seed whose .git is a symlink');
  }
  if (!gitDirStat.isDirectory()) {
    // A file-form `.git` is a gitdir pointer into an external repository —
    // exactly the external reference sealed probes must never resolve.
    throw new SealedProbeStagingError('Refusing to stage a seed whose .git is not a directory');
  }
...
async function stageOneSeed(
  ...
  if (fs.existsSync(seedPath)) {
    throw new SealedProbeStagingError(`Seed directory already exists: ${seedId}`);
  }
 ...
 * {@link SealedProbeStagingError} is thrown so the caller aborts before the
 * primary agent starts.
 */
export async function stageSealedProbeSeeds(
  params: StageSealedProbeSeedsParams,
): Promise<SealedProbeStagingResult> {
  ...
  try {
    ...
    throw new SealedProbeStagingError(`Sealed-probe staging failed: ${message}`);
  } finally {
    ...
    // for the broker, the agent, or a later phase to find.
  ...
  for (const seed of seeds) {
    logger.debug(`Sealed probes: staged ${seed.repo} at ${seed.commit} (seed ${seed.seedId})`);
  }
