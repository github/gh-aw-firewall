...
/** Environment variable the askpass helper reads the staging token from. */
const ASKPASS_TOKEN_ENV = 'AWF_SEALED_PROBE_STAGING_TOKEN';
 ...
 *
 * The helper reads the token from its own environment; nothing sensitive is
 * written to disk. The file is created 0700 inside the (already 0700) sealed
 * probe root.
 */
function writeAskpassHelper(root: string): string {
  ...
  const script = [
    '#!/bin/sh',
    '# Generated by AWF sealed-probe staging. Reads the credential from the',
    '# environment so it never appears in argv or on disk.',
    'case "$1" in',
...
export function buildStagingGitEnv(params: {
  ...
  return {
    ...
    GIT_CONFIG_VALUE_0: '',
    [ASKPASS_TOKEN_ENV]: params.token,
  };
}
...
export async function stageSealedProbeSeeds(
  ...
  const gitEnv = buildStagingGitEnv({ token, askpassPath, isolatedHome });
  ...
  try {
    ...
    // The helper is only needed for the duration of staging. Removing it (and
    // the isolated HOME) leaves no staging artifact behind for the broker,
    // the agent, or a later phase to find.
